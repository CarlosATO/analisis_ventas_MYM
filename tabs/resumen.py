...
def render_resumen(
    ...
    if not weekly_e.empty:
        ...
        if not det.empty:
            ...
            det_show["Margen"] = det_show["Margen"].apply(fmt_money)
            st.write(f"**Productos vendidos en {sel_label}:**")
            st.dataframe(det_show, hide_index=True, width="stretch")
            dl_excel_btn(det_show, f"detalle_semana_{sel_anio}_{sel_sem}.xlsx", "Semana")
        else:
